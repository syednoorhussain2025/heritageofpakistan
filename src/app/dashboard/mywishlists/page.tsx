...
import { useWishlists, fetchWishlists, dashboardKeys } from "@/hooks/useDashboardQueries";
import { useQueryClient } from "@tanstack/react-query";
import { useState } from "react";

export default function MyWishlistsPage() {
  const { data: lists = [], isLoading } = useWishlists();
  ...
  async function handleDelete(id: string, name: string) {
    ...
    try {
      ...
      queryClient.setQueryData(dashboardKeys.wishlists("me"), (old: any[]) =>
  ...
  const filtered = useMemo(() =>
    q.trim() ? lists.filter((w: any) => w.name.toLowerCase().includes(q.trim().toLowerCase())) : lists,
    [lists, q]
  );
  ...
  return (
    <>
      ...
      {filtered.length === 0 && !isLoading && q.trim() && (
