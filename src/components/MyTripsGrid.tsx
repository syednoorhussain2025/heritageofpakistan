...
type MyTripsGridProps = {
  ...
  /**
   * Layout:
   * - "page": full framed page section (bg, container, breadcrumb, title)
   * - "embedded": drop-in section for dashboard/right-pane (no outer chrome)
   */
  variant?: "page" | "embedded";
  /** Optional: override what happens when a card is clicked */
  onOpenTrip?: (slug: string) => void;
  /** Optional: show/hide delete actions (default true) */
  allowDelete?: boolean;
  /** Optional heading override (default: "Your Trips") */
  title?: string;
};
...
export default function MyTripsGrid({
  ...
  const goToTrip = (slug: string) => {
    if (onOpenTrip) return onOpenTrip(slug);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Fetch item counts (non-blocking)
        Promise.all(
          ...
          {filtered.map((t) => {
            ...
            return (
              <article
                ...
                role="button"
                tabIndex={0}
                onClick={() => goToTrip(t.slug)}
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") goToTrip(t.slug);
                }}
