 ...
 * Used for cover uploads before saving metadata into site_images.
 */
...
function ListingForm({
  ...
  async function deleteCover() {
    ...
    try {
      ...
      // also delete corresponding site_images row, if any
      await supabase
        .from("site_images")
        .delete()
        .eq("site_id", site.id)
...
function CoverLibraryModal({
  ...
  async function onUpload(files?: FileList | null) {
    ...
    try {
      ...
      for (const f of queue) {
        // Extract client-side meta (dimensions + blur)
        ...
        // Insert metadata into site_images
        ...
        if (dbError) {
          // Log but don't block the UI; storage upload already succeeded.
          console.error("Failed to insert cover into site_images", dbError);
        }
      }
      await refresh();
    } catch (e) {
  ...
  async function deleteOne(key: string) {
    ...
    try {
      ...
      // Delete matching site_images row if present
      await supabase
        .from("site_images")
        .delete()
        .eq("site_id", siteId)
  ...
  async function deleteAll() {
    ...
    try {
      ...
      // Delete all corresponding site_images rows for this site & these paths
      await supabase
        .from("site_images")
        .delete()
        .eq("site_id", siteId)
        .in("storage_path", keys);
