...
// page-local imports from our heritage folder
...
/* ---------------- Types for server-provided site ---------------- */
...
type HeritageClientSite = {
  ...
  cover?: HeroCover;
  // The hook will add many more fields (history_layout_html, etc.)
  [key: string]: any;
};
...
export default function HeritagePage({
  ...
  // Slug comes from server-provided site
  ...
  const {
    ...
    setHighlight,
    maps,
    travelGuideSummary, // ✅ from hook
  } = useHeritageData(slug, deepLinkNoteId);

  // ✅ Merge server site (SSR) with fetched site (client)
  // - fetchedSite wins for content fields
  // - cover is merged so SSR blurDataURL is preserved and client can add/override fields
  const site: HeritageClientSite | null = (() => {
    ...
    const mergedCover =
      initialSite?.cover || (fetchedSite as any)?.cover
        ? {
            ...(initialSite?.cover || {}),
            ...((fetchedSite as any)?.cover || {}),
          }
        : undefined;
  ...
  return (
    ...
    <CollectionsProvider>
      <div className="min-h-screen bg-[#f8f8f8]">
        {/* HERO — SSR site immediately; then hydrated cover/blur from client */}
        {!site ? (
          <HeroSkeleton />
        ) : (
          <HeritageCover
            ...
            fadeImage={true} // ✅ ensure blur → image fade is active
          />
        )}
              ...
              <>
                ...
                {Array.isArray(site.custom_sections_json) &&
                  site.custom_sections_json
                    .filter(
                      (cs: any) =>
                        !!cs.layout_html && cs.layout_html.trim() !== ""
                    )
                    .map((cs: any) => (
                      <HeritageSection
                        key={cs.id}
